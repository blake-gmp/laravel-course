...
<div class="container pt-5">
    <div class="row">
                            ...
                            <div class="card h-100 border-0">
                                <div class="card-img-top">
                                    @if(!is_null($product->image_path))
                                        <img src="{{ asset('storage/'.$product->image_path) }}" width="240" height="240" class="img-fluid mx-auto d-block" alt="Card image cap">
                                    @else
        ...
        <form class="col-md-4 order-md-1 col-lg-3 sidebar-filter">
            <h3 class="mt-0 mb-5"><i class="fa-solid fa-clipboard-list"></i> {{__('products.welcome.products')}} <span class="text-primary ">{{ count($products) }}</span></h3>
            <h6 class="text-uppercase font-weight-bold mb-3">{{__('products.welcome.categories')}}</h6>
            ...
            <div class="divider mt-5 mb-5 border-bottom border-secondary"></div>
            <a href="#" class="btn btn-lg btn-block btn-primary mt-5" id="filter-button"><i class="fa-solid fa-filter"></i> {{__('products.welcome.filter')}}</a>
        </form>
...
@section('javascript')
    const WELCOME_DATA = {
        ...
        defaultImage: '{{ asset( $defaultImage) }}/',
        addToCart: '{{ url('cart') }}/',
        listCart: '{{ url('cart') }}',
        isGuest: '{{ $isGuest }}'
    }
